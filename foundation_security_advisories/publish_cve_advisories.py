...
def try_update_published_cve(local_cve: CVEAdvisory, local_date: int, remote_date):
    ...
    # If there are references with tags attached, we did not add them automatically,
    # and probably also don't want to remove them automatically, so we move them to
    # our to-be-published object.
    # Old advisories still sometimes have tags like `x_refsource_MISC`, we do not
    # want those.
    remote_extra_references = list(
        filter(
            lambda reference: "tags" in reference
            and any(not tag.startswith("x_") for tag in reference["tags"]),
